...
class Silkbot {
  ...
  constructor(client, config, storage) {
    ...
    // this._client.on('message', async m => this.setup(m));
    ...
    this._client.guilds.cache.forEach(g => {
      ...
      g.channels.cache.forEach(async (c) => {
        ...
        try {
          ...
          msgs.filter(m => {
            return m.author.id == this._client.user.id &&
              m.mentions.roles.size > 0
          }).forEach(m => {
            console.log(`${m.content}`)
          })
        } catch (err) {
          console.log(`Unable to fetch for ${c.name} ... ${err.message}`)
        }
      });
    });
  ...
  async setup(message) {
    ...
    // console.log(emoteRegex.exec(message))
    // console.log(animatedEmoteRegex.exec(message))
  ...
  async messageReactionRemove(reaction, user) {
    ...
    if (user.bot) {
      console.log('Message reaction from bot, ignore');
      return;
    }

    if (!message.author.bot) {
      console.log('Message reaction not bot related, ignore');
      return;
    }
    
    if (message.author.id !== this._client.user.id) {
      console.log('Message not owned by silkbot');
      return;
    }
    ...
    try {
      ...
      if (memberHasRole) {
        await testMember.roles.remove(mentionedRole);
  ...
  async messageReactionAdd(reaction, user) {
    ...
    try {
      ...
      if (memberHasRole) {
        const dm = await member.createDM();
        dm.send(`You have already been assigned the role "${mentionedRole.name}" within the guild "${guild.name}".`);
      } else {
        await testMember.roles.add(mentionedRole);
        const dm = await member.createDM();
