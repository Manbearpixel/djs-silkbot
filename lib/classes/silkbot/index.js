  ...
  async onReady() {
    ...
    this._client.guilds.cache.forEach(g => {
      g.channels.cache.forEach(c => {
        ...
        channelMessages.push(c.messages.fetch({ limit: 50 }));
      });
    });

    Promise.all(channelMessages)
      .then((cMessages) => {
        try {
          const silkbotPosts = cMessages.reduce((allMsgs, msgs, idx) => {
            allMsgs.push(msgs.filter(m => m.author.id == this._client.user.id).array());
            return allMsgs.flat();
          }, []);
          this.logInfo(`Loaded ${silkbotPosts.length} messages.`);
        } catch (err) {
          this.logError(`Unable to preload Silkbot messages.
          Message: ${err.message}\n`);
        }
      });
