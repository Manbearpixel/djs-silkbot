...
class Silkbot {
  ...
  async bouncerReaction(reaction, guildMember) {
    ...
    const dmChannel = await guildMember.createDM();
    if (memberHasRole) {
      await dmChannel.send(`Verification for ${guild.name} not required. You alreadyare assigned the correct role.`);
      return;
    }
  ...
  async messageReactionAdd(reaction, user) {
    ...
    if (/bouncer/ig.test(message.content)) return await this.bouncerReaction(reaction, member);
    try {
      const memberHasRole = member.roles.cache.some(r => r.id == mentionedRole.id);
      if (memberHasRole) {
        const dm = await member.createDM();
