...
class Cache {
   ...
   */
  async pushToCache(item, debug) {
    if (debug) return this.debugItem(item);
    const temp = Cache.deepCloneArr(this._cache);
  ...
  /**
   * Returns a direct reference to an `item` stored within the internal `cache`.
   * 
   * @param {*} index 
   */
  getItemRef(index) {
